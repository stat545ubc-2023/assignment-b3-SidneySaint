...
data("USArrests") #[2] (Numbers in square brackets are references - full references below) # crime and population data by state for 1976.
## EDIT 4B 
...
# in the different US states. One tab displays crime and population density from 1976. The second shows updated data from 2015. 
...
ui <- navbarPage("Violent Crimes and Population Density by Time in the USA", theme = shinytheme("journal"), #[17]
                 # Add a tab for older 1976 data (the only data used for Assignment 3B submission)
                 tabPanel("1976",
                          # Add a sidebar panel to the tabs for graphs 
                          sidebarLayout(
                            sidebarPanel(
                              #4B ADDITIONS: 'MAP_Plot*' Features: Reactive Maps of US States, with urban populations (a), Murders (b) , and assualt rates (c) c. 1976 overlayed
                            ...
                            mainPanel(
                              ...
                              h5("This tab shows crime and population data from 1976."),
                              ...
                              textOutput("ReferencesTAB1_1976"),
                              width = 7
                            )
                          )
                 ),
...
server <- function(input, output) {
  ...
  ## 4B ADDITION: filter the c. 1976 data + USA geo-spatial data - filtered by user input on the single slider included on UI. 
  ...
  ## Retained 3B feature: A plot depicting how 1976 murder rates as a function of assaults across states; based on filtered dataset and therefore reactive to UI filtering of data based on urban population of state. Useful to for visualizing the Murder ~ Assualt realtionship and how it might change based on % urbanization of state.
  ...
  # Retained 3B data table feature. An interactive data table (by DT, calling the filtered 1976 data table.) I think the biggest utility this feature adds is that is allows users to search for a specific US state's crime data in the search bar.
  output$CrimeData <- DT::renderDataTable({
    ArrestsFiltered()
  })
  
  #  Retained 3B feature (text updated to reflect 4B submission); Text to go at the bottom of the app: explains the units of 1976 crime statistics displayed in the interactive data table. References the data source and image source.
  output$ReferencesTAB1_1976 <- renderText({
  ...
  ## 4B ADDITIONS: Reactive *maps* of 1976 crime and population density data:
  
  # below for: adding a state label in each graph  / get mean coordinates per state to include as labels on MAP_Plot* reactive map features (Tab1; 1976 data)
  ...
  ## (4B ADDITION): define map of US states (spatial coordinates are x and y; shape is polygon) with urban population c 1976 
  ...
  ## (4B ADDITION): define map of US states with Murder rate c 1976 indicated by colour of state. 
  ...
  ## (4B ADDITION): define map of US states with assault rate c 1976 indicated by colour of state. 
...
# For introducing me to the 'usmap' package and how to plot that data and US data from map_data. (Used for reactive maps c. 1976 data and 2015 data maps)
